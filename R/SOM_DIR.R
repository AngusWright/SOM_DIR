...
while (length(inputs)!=0) {
  ...
  if (inputs[1]=='-p') { 
    ...
    inputs<-inputs[-1]
    #/*fend*/}}}
  } else if (inputs[1]=='--no.whiten.features') {
...
if (length(factor.label)<2) { 
  ...
  if (reuse && file.exists(som.datfile)) {
    ...
    train.som<-kohtrain(data=train.cat,train.expr=factor.label,som.dim=som.dim,som.topo=som.topo,
                        ...
                        train.sparse=sparse.som,sparse.frac=sparse.frac,sparse.min.density=sparse.min.density,sparse.var=sparse.var,
                        data.missing=data.missing,data.threshold=data.threshold,quiet=quiet,seed=seed,whiten.input=whiten.input)
    #/*fend*/}}}
